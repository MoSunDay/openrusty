...
- workspace 包含六个 crate：`openrusty-core`（config、请求上下文、phase 与 Decision 语义）、`openrusty-wasm`（wasmtime 运行时：ABI imports、沙箱 runner、热重载 registry）、`openrusty-proxy`（upstream、负载均衡、被动健康、连接池、转发）、`openrusty-server`（`openrusty` 二进制：同端口 HTTP/1.1+h2c、阶段管线、`/openrusty/*` 端点）、`openrusty-sdk`（`no_std` 插件 SDK）、`openrusty-macros`（`#[phase(...)]` 过程宏）。
- `plugins/` 下是两个各自独立 workspace 的一方插件（`wasm32-unknown-unknown` cdylib）：`kv-scheduler`（vLLM 风格 KV-cache 亲和调度）与 `kv-probe`（覆盖全部 8 个阶段的 e2e 探针，供集成演练使用）。
