...
## 职责
- 上游模型与转发层：upstream/peer 定义、负载均衡算法、被动健康检查、连接池客户端、HTTP 转发与重试、WebSocket 字节隧道。

## 边界
- 负责：`swrr`/`ip_hash` 选择算法（`balancer.rs`）；被动健康的记账与判定（`health.rs`）；按地址池化的客户端（`client.rs`）；转发、`X-Forwarded-For` 合并、可重试错误分类（`forward.rs`）；双向隧道（`tunnel`）。
...
- 对外接口：`forward`、`tunnel`、`swrr_next`/`ip_hash_pick`、`HealthRegistry` 系列（`register`/`healthy_indices`/`record_failure`/`record_success`）、`ClientPool`。
