...
## 关键设计
- 路由表：`/openrusty/status`（GET）与 `/openrusty/reload`（POST）先于业务路由匹配，其余请求进入 `handle_request` 代理管线。
...
- 对外接口：监听端口上的全部网关行为 + `GET /openrusty/status` + `POST /openrusty/reload`。
- 代码锚点：`crates/openrusty-server/src/{main,h2c,app,pipeline,ws,body_filter,reload}.rs`。
