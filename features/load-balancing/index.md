...
## 能力概述
- 每个 upstream 在多个 peer 之间分发流量，支持平滑加权轮询、客户端 IP 固定与最少连接三种策略；被动健康检查自动摘除故障 peer 并在冷却后恢复，可选主动健康探测周期性探活全部 peer（含无流量 peer）。

## 触发方式
- 由转发路径自动驱动（每次 upstream 尝试前选一个健康 peer）；策略在 `config/openrusty.toml` 的 `[[upstreams]].balancer` 配置：`swrr` | `ip_hash` | `least_conn`。
...
- `least_conn`：每次选取「在途请求数 / weight」最小的健康 peer（整数交叉相乘比较，免浮点；并列取最低下标）；在途计数按 peer 地址记在健康注册表里，跨热重载保留。
...
## 关键状态与异常
- 状态：每 (upstream, peer) 的失败计数、down 标记、恢复时间、在途请求计数（`least_conn` 的输入），主动探测另记每 peer 连续成功/失败计数；按 upstream 名/peer 地址键控、跨热重载保留。
