...
- 三层验证：`cargo test --workspace`（单元）；`scripts/build-plugins.sh`（插件单测 + wasm 构建）；`scripts/integration.sh`（e2e 演练，93 checks，覆盖代理/流式协议、调度、热重载、被动+主动健康、retry_on_timeout、收容、路由超时、kv-probe 全阶段、status/metrics 形状、env 配置启动等）。
